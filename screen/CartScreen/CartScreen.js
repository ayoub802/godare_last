...
const CartScreen = (props) => {
  ...
  const DeleteBasket = useCallback( async() => { 
    ...
    ToastAndroid.show('Le panier a été vidé', ToastAndroid.SHORT)
    setLoader(false);
  },[]);
  ...
  async function removeProductFromCart(item){
    ...
    ToastAndroid.show('Produit supprimé du panier', ToastAndroid.SHORT)
  };
  ...
  const RenderAttribute = props => {
    ...
    return (
     <>
      <Text style={styles.WeightCalSubText}>
        Etat : {('Used' == product.stateValue ? t('Occasion') : t('Neuf') )}
      </Text>
      <Text style={[styles.WeightCalSubText, {marginTop: 8}]}>
        {product.productValue ? (t('Valeur') + ' : ' + product.productValue) : ''}
      </Text>
     </>
          ...
          (
            ...
            <View style={{paddingHorizontal: 48}}>
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center"}}>
                   ...
                   <Text style={{color: "#000", fontSize: wp(3.4),fontFamily: "Poppins-SemiBold", letterSpacing: 0.3}}>{ subTotal } €</Text>
                </View>
