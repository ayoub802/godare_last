...
import { removePanier, savePanier, getAuthUserEmail, getPanier, getParametrages } from '../../modules/GestionStorage';
...
const ByPlaneDetailsComponent = (props) => {
  ...
  useEffect(() => {
    ...
    //   console.log("Count of the basket :", basketData.length);
  ...
  const showDouaneMessage = async (item) => {
    ...
    console.log("Parametrages :", parametrages);
    ...
    if ('New' == item)
    {
        ...
        {
            return ToastAndroid.show(parametrages.messageFraisDouane, ToastAndroid.SHORT)
        }
    }
    ...
    if ('Used' == item)
    {
        ...
        {
            return ToastAndroid.show(parametrages.messageFraisUsageDouane, ToastAndroid.SHORT);
        }
    }
  ...
  const formatCurrency = (value) => {
    ...
    const [integerPart, decimalPart] = formattedInput.split(/[.,]/);
    ...
    const formattedValue = `${integerPart || ''}${formattedDecimalPart !== '' ? '.' : ''}${formattedDecimalPart}`;
  ...
  const handleInputChange = (input) => {
    // Utiliser une expression régulière pour valider et formater l'entrée
    const formattedInput = input.replace(/[^0-9.,]/g, ''); // Garder uniquement les chiffres, virgules et points
    const [integerPart, decimalPart] = formattedInput.split(/[.,]/);

    // Limiter la partie décimale à deux chiffres
    const formattedDecimalPart = decimalPart ? decimalPart.slice(0, 2) : '';

    // Concaténer la partie entière et la partie décimale
    const formattedValue = `${integerPart || '0'}${formattedDecimalPart !== '' ? '.' : ''}${formattedDecimalPart}`;
      ...
      <View style={{flexDirection: "row", alignItems: "center",gap: 10, paddingVertical: 12, paddingLeft: 22}}>
        ...
        <ScrollView
          ...
          <View style={[styles.inputContainer, {position: "relative", zIndex: -10}]}>
            ...
            />
            {value !== '' && (
              <Text>{parseFloat(value).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</Text>
            )} */}
          </View>
